...
async def game_give_role(chat_id, user_id, role):
    try:
        ...
        file = os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + str(user_id) + ".txt"
        player = open(file, "+w")
...
async def set_mafia_mode(chat_id, day, mode, message, start=False):
    try:
        file = os.getcwd() + "/chats/" + str(chat_id) + "/info.txt"
        if not start:
            try:
                game = open(file)
        ...
        if mode == "Night":
                ...
                for temp in players:
                    ...
                    if int(info[0]) != 4:
                        ...
                        for item in live_players:
                            player = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + item)
            ...
            try:
                ...
                for temp in players:
                    ...
                    player = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + temp, "+w")
                ...
                if killed != medic_help and killed != 0:
                    kill = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + str(killed) + ".txt")
                    info = kill.read().split("|")
                    kill.close()

                    kill = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + str(killed) + ".txt", "+w")
            ...
            try:
                ...
                for item in players:
                    ...
                    buttons = []
                    file = os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + item
                    player = open(file)
                    ...
                    for temp in players:
                        file = os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + temp
                        player = open(file)
                        records = player.read().split("|")
                        player.close()
            ...
            try:
                ...
                if voted:
                    counter = 0
                    killed = voted[0] 
                    for i in len(range(voted)):
                        count = 0
                        for a in len(range(voted)):
                            if a != i and voted[a] == voted[i]:
                                count += 1
                        if count > max_count:
                            max_count = count
                            killed = voted[i]
                ...
                game_message = ""
                if killed != 0:
                    player = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + str(killed) + ".txt")
                    get = player.read().split("|")
                    player.close()

                    game_message = "🍍 *Мафия*\n*Голосование*\n\nГолосование было завершено\n\n💀 [%s](tg://user?id=%d) был(-а) подвешен(-а) на вилисице!" % (get[3], killed) 

                    player = open(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + str(killed) + ".txt", "+w")
...
def destroy_mafia(chat_id):
    try:
        ...
        for temp in players:
            os.remove(os.getcwd() + "/chats/" + str(chat_id) + "/mafia/" + temp)
            try:
                file = open(os.getcwd() + "/users/" + temp)
                chat_user = int(file.read())
                file.close()
...
async def leave_command(message: types.Message):
    try:
        ...
        try:
            ...
            game = open(os.getcwd() + "/chats/" + str(chat) + "/info.txt")
            ...
            if "MAFIA" == game_result[0]:
                if "REGISTER" == game_result[1]:
                    ...
                    player = open(os.getcwd() + "/chats/" + str(chat) + "/mafia/" + str(message.from_user.id) + ".txt")
                    get = player.read().split("|")
                    player.close()

                    killed = open(os.getcwd() + "/chats/" + str(chat) + "/mafia/" + str(message.from_user.id) + ".txt", "+w")
...
async def some_callback_handler(callback_query: types.CallbackQuery):
    try:
        ...
        if code == "Игры":
            ...
            try:
                file = os.getcwd() + "/users/" + str(callback_query.from_user.id) + ".txt"
                player = open(file)
                info = player.read()
                player.close()
                ...
                file = os.getcwd() + "/chats/" + str(callback_query.message.chat.id) + "/mafia/" + str(callback_query.from_user.id) + ".txt"
                game_message = "🍍 *Мафия*\n\nУчастники:\n"
                player = open(file, "+w")
                player.close()
            ...
            file = os.getcwd() + "/chats/" + str(callback_query.message.chat.id) + "/crosses/" + str(callback_query.message.message_id) + ".txt"
            game = open(file, "w+")
            ...
            if callback_query.message.chat.id == callback_query.from_user.id:
                try:
                    ...
                    file = open(os.getcwd() + "/users/" + str(callback_query.from_user.id) + ".txt")
                    chat_user = file.read()
                    file.close()

                    game = open(os.getcwd() + "/chats/" + chat + "/info.txt")
                    ...
                    player = open(os.getcwd() + "/chats/" + chat + "/mafia/" + str(callback_query.from_user.id) + ".txt")
                    info = player.read().split("|")
                    player.close()

                    player = open(os.getcwd() + "/chats/" + chat + "/mafia/" + index + ".txt")
                    result = player.read().split("|")
                    player.close()

                    if int(result[1]) != 0:
                        file = os.getcwd() + "/chats/" + chat + "/mafia/" + str(callback_query.from_user.id) + ".txt"
                        player = open(file, "+w")
